@extends('header') @section('head') @parent @include('money_script')
...
{!! Former::open($url) ->addClass('warn-on-exit main-form') ->onsubmit('return onFormSubmit(event)') ->method($method) !!}
    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-md-6">
                ...
                {!! Former::select('vendor_id')->addOption('', '') ->label(trans('texts.vendor')) ->addGroupClass('vendor-select') !!} 
                <div class="form-group" id="supplier-modal" style="display:none">
                    <button onclick="return false" type="button" class="btn" data-toggle="modal" data-target="#suppliersModal">Enter Supplier Details</button>
                </div>
                @if($expense) 
                    {!! Former::populateField("tags",explode(",",trim($expense->tags,","))) !!} 
                @endif 
                ...
                {!! Former::select('expense_category_id')->addOption('','') ->label(trans('texts.category')) ->addGroupClass('expense-category-select') !!} 
                {!! Former::text('amount') ->label(trans('texts.amount')) ->data_bind("value: amount, valueUpdate: 'afterkeydown'") ->addGroupClass('amount') ->append('<span data-bind="html: expenseCurrencyCode"></span>') !!} 
                {!! Former::select('expense_currency_id')->addOption('','') ->data_bind('combobox: expense_currency_id') ->label(trans('texts.currency_id')) ->data_placeholder(Utils::getFromCache($account->getCurrencyId(),'currencies')->name) ->fromQuery($currencies, 'name', 'id') !!} 
                
                @if (! $isRecurring) 
                    {!! Former::text('expense_date') ->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT, DEFAULT_DATE_PICKER_FORMAT)) ->addGroupClass('expense_date')->label(trans('texts.date')) ->append('<i class="glyphicon glyphicon-calendar"></i>') !!} 
                @endif 
                
                @if ($expense && $expense->invoice_id) 
                    {!! Former::plaintext() ->label('client') ->value($expense->client->present()->link) !!} 
                @else 
                    {!! Former::select('client_id') ->addOption('','') ->label(trans('texts.client')) ->data_bind('combobox: client_id') ->addGroupClass('client-select') !!} 
                @endif 
                
                @if (count($taxRates)) 
                    @if (!$expense || ($expense && (!$expense->tax_name1 && !$expense->tax_name2))) 
                        {!! Former::checkbox('apply_taxes')->text(trans('texts.apply_taxes')) ->data_bind('checked: apply_taxes') ->label(' ') ->value(1) !!} 
                    @endif 
                @endif

                <div style="display:none" data-bind="visible: apply_taxes">
                    <br/> @include('partials.tax_rates')
                </div>

                @if (!$expense || ($expense && !$expense->invoice_id)) 
                    {!! Former::checkbox('should_be_invoiced') ->text(trans('texts.mark_billable')) ->data_bind('checked: should_be_invoiced()') ->label(' ') ->value(1) !!} 
                @endif 
                
                @if ($isRecurring) 
                    {!! Former::select('frequency_id')->label('frequency') ->options(\App\Models\Frequency::selectOptions()) ->data_bind("value: frequency_id") !!} 
                    {!! Former::text('start_date') ->data_bind("datePicker: start_date, valueUpdate: 'afterkeydown'") ->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT,DEFAULT_DATE_PICKER_FORMAT)) ->appendIcon('calendar') ->addGroupClass('start_date') !!} 
                    {!! Former::text('end_date') ->data_bind("datePicker: end_date, valueUpdate: 'afterkeydown'") ->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT,DEFAULT_DATE_PICKER_FORMAT)) ->appendIcon('calendar') ->addGroupClass('end_date') !!} 
                @else 
                
                    @if ((! $expense || ! $expense->transaction_id)) 
                        @if (! $expense || ! $expense->isPaid()) 
                            {!! Former::checkbox('mark_paid') ->data_bind('checked:mark_paid') ->text(trans('texts.mark_expense_paid')) ->label(' ') ->value(1) !!} 
                        @endif

                        <div style="display:none" data-bind="visible: mark_paid">
                            {!! Former::select('payment_type_id') ->addOption('','') ->fromQuery($paymentTypes, 'name', 'id') ->addGroupClass('payment-type-select') !!} 
                            {!! Former::text('payment_date') ->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT)) ->addGroupClass('payment_date')->append('<i class="glyphicon glyphicon-calendar"></i>') !!} {!! Former::text('transaction_reference') !!}
                        </div>
                    @endif 
                    @if (!$expense || ($expense && ! $expense->isExchanged())) 
                        {!! Former::checkbox('convert_currency') ->text(trans('texts.convert_currency')) ->data_bind('checked: convert_currency') ->label(' ') ->value(1) !!} 
                    @endif
                    ...
                    <div style="display:none" data-bind="visible: enableExchangeRate">
                        ...
                        <span style="display:none;" data-bind="visible: client_id">
                                    {!! Former::plaintext('') ->value('<span data-bind="html: invoiceCurrencyName"></span>') ->style('min-height:46px') ->label(trans('texts.invoice_currency')) !!}
                        </span>

                        {!! Former::text('exchange_rate') ->data_bind("value: exchange_rate, enable: enableExchangeRate, valueUpdate: 'afterkeydown'") !!} 
                        {!! Former::text('invoice_amount') ->addGroupClass('converted-amount') ->data_bind("value: convertedAmount, enable: enableExchangeRate")->append('<span data-bind="html: invoiceCurrencyCode"></span>') !!}
                    </div>

                    @if ($account->hasFeature(FEATURE_DOCUMENTS)) 
                        {!! Former::checkbox('invoice_documents') ->text(trans('texts.add_documents_to_invoice')) ->onchange('onInvoiceDocumentsChange()') ->data_bind('checked: invoice_documents') ->label(' ') ->value(1) !!} 
                    @endif
                @endif
            ...
            <div class="col-md-6">
                ...
                @if (! $isRecurring && $account->hasFeature(FEATURE_DOCUMENTS))
                    <div class="form-group">
                        <label for="public_notes" class="control-label col-lg-4 col-sm-4">{{trans('texts.documents')}}</label>
    ...
    <div class="modal-dialog" style="min-width:150px">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="suppliersModalLabel">{{ trans('texts.supplier') }}</h4>
            </div>
                ...
                <div class="row">
                            <div class="col-md-12">
                                <div >
                                    ...
                                    <div class="">
                                        {!! Former::text('supplier[suffix]')->label("suffix") !!} 
                                        {!! Former::text('supplier[legal_business_name]')->label("legal_business_name") !!} 
                                        {{  Former::populateField("supplier[type]","Supplier")  }} 
                                        {!! Former::hidden('supplier[type]') !!} 
                                        {!! Former::select('supplier[region]')->label("region")->options(["GVRD","FVRD","RMOW","CRD","SCRD","SLRD","RDOS","GTA","SF"]) !!} 
                                        {!! Former::select('supplier[status]')->label("status")->options(["Active","Closed","Inactive","Suspended"]) !!} 
                                        {!! Former::text('supplier[work_phone]')->label("work_phone") !!} 
                                        {!! Former::text('supplier[phone_home]')->label("phone_home") !!} 
                                        {!! Former::text('supplier[phone_main]')->label("phone_main") !!} 
                                        {!! Former::text('supplier[phone_fax]')->label("phone_fax") !!} 
                                        {!! Former::text('supplier[phone_toll_free]')->label("phone_toll_free") !!} 
                                        {!! Former::text('supplier[phone_cell]')->label("phone_cell") !!} 
                                        {!! Former::text('supplier[email]')->label("email") !!} 
                                        {!! Former::text('supplier[website]')->label("website") !!} 
                                        {!! Former::text('supplier[keywords]')->label("keywords") !!} 
                                        {!! Former::text('supplier[id_number]')->label("id_number")->placeholder($account->clientNumbersEnabled() ? $account->getNextNumber() : '') !!} 
                                        {!! Former::text('supplier[vat_number]')->label("vat_number") !!}
                                    </div>
                                </div>
                            </div>
                        </div>
...
<center class="buttons">
    {!! Button::normal(trans('texts.cancel')) ->asLinkTo(HTMLUtils::previousUrl('/expenses')) ->appendIcon(Icon::create('remove-circle')) ->large() !!} 
    @if (Auth::user()->canCreateOrEdit(ENTITY_EXPENSE, $expense)) 
        @if (Auth::user()->hasFeature(FEATURE_EXPENSES))
            @if (!$expense || !$expense->is_deleted) 
                {!! Button::success(trans('texts.save'))->appendIcon(Icon::create('floppy-disk'))->large()->submit() !!} 
            @endif 
            @if ($expense && !$expense->trashed()) 
                {!! DropdownButton::normal(trans('texts.more_actions'))->withContents($actions) ->large() ->dropup() !!} 
            @endif 
            @if ($expense && $expense->trashed()) 
                {!! Button::primary(trans('texts.restore'))->withAttributes(['onclick' => 'submitAction("restore")'])->appendIcon(Icon::create('cloud-download')) ->large() !!} 
            @endif 
        @endif 
    @endif
</center>
