...
function voidInvoice(id) {
    $("#viodInvoiceModal form").attr("action", "/invoices/" + id + "/voided");
    $("#viodInvoiceModal").modal();
}
...
function calculateAmounts(invoice) {
    ...
    for (var i = 0; i < invoice.invoice_items.length; i++) {
        ...
        if (_cost < 0) {
            ...
            if (cost && cost.indexOf("%") !== -1) {
                ...
                lineTotal = roundToTwo(discount);
                paritial_total = 0;

            } else {

                lineTotal = roundToTwo(_cost);
            }
        } else {
    ...
    for (var i = 0; i < invoice.invoice_items.length; i++) {
        ...
        if (NINJA.parseFloat(item.cost) < 0) {

            if (item.cost && item.cost.indexOf("%") !== -1) {
                cost = NINJA.parseFloat(item.cost);
                var discount = cost / 100 * paritial_total;
                lineTotal = roundToTwo(discount);
                paritial_total = 0;
                ...
                lineTotal = roundToTwo(item.cost);
                paritial_total = 0;
            }
        } else {
            paritial_total = lineTotal;
    ...
    var interest = 0;
    if (invoice.interest == 1 && TODAY_DATE && invoice.balance > 0) {
        var invoice_date = new Date(invoice.invoice_date);
        var due_date = new Date(invoice.due_date);
        var today_date = new Date(TODAY_DATE);
        var days = (today_date - invoice_date) / (1000 * 60 * 60 * 24);
        if (days > 30) {
            invoice.interest_rate = interest = roundToTwo(0.1 * (total - (roundToTwo(invoice.amount) - roundToTwo(invoice.balance))) / 100 * days)
        }
    } else if (invoice.interest_paid > 0) {
        invoice.interest_rate = interest = invoice.interest_paid;
    }
    invoice.total_amount = roundToTwo(roundToTwo(total) + interest - (roundToTwo(invoice.amount) - roundToTwo(invoice.balance)));
